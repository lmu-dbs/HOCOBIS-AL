...
def warmup_learning_rate(optimizer, curr_step, warmup_step, lr):
...
def adjust_learning_rate(optimizer, epoch, lr, warmup_epoch, epochs, schedule, cos):
    """Decay the learning rate based on schedule"""
    if cos:  # cosine lr schedule
        progress = float(epoch - warmup_epoch) / float(epochs - warmup_epoch)
